...
@Service
@Transactional
class PurchaseOrderFreightCommandHandler(
    ...
    fun handle(command: UpdateFreightCommand) {
        log.debug("Handling UpdateFreightCommand purchaseOrderId={}", command.id)
        val order = repository.findByIdOrThrow(command.id)
        order.updateFreight(command)
