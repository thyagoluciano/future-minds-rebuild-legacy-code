...
@Service
@Transactional
class PurchaseOrderSubscriptionCommandHandler(
    ...
    fun handle(command: UpdateSubscriptionCommand) {
        log.debug("Handling UpdateSubscriptionCommand purchaseOrderId={}", command.id)
        val order = repository.findByIdOrThrow(command.id)
        order.updateSubscription(command)
