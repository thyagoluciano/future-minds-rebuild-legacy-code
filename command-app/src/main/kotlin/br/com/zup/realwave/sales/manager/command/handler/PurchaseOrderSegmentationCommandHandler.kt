...
@Service
@Transactional
class PurchaseOrderSegmentationCommandHandler(
    ...
    fun handle(command: UpdateSegmentationCommand) {
        log.debug("Handling UpdateSegmentationCommand purchaseOrderId={}", command.id)
        val order = repository.findByIdOrThrow(command.id)
        order.updateSegmentation(command)
