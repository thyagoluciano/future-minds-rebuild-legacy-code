...
@Service
@Transactional
class PurchaseOrderPaymentCommandHandler(
    ...
    fun handle(command: UpdatePaymentCommand) {
        log.debug("Handling UpdatePaymentCommand purchaseOrderId={}", command.id)
        val order = repository.findByIdOrThrow(command.id)
        order.updatePayment(command)
