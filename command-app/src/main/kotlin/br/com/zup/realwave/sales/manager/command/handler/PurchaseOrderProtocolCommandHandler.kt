...
@Service
@Transactional
class PurchaseOrderProtocolCommandHandler(
    ...
    fun handle(command: UpdateProtocolCommand) {
        log.debug("Handling UpdateProtocolCommand purchaseOrderId={}", command.id)
        val order = repository.findByIdOrThrow(command.id)
        order.updateProtocol(command)
